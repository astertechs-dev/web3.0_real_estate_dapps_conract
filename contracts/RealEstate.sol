...
contract RealEstate {
    ...
    event ReviewAdded(uint256 indexed productId, address indexed reviewers, uint256 rating, string comment);
    ...
    function getProdctReviews() external view returns (Review[] memory) {}
    ...
    function likePreview(uint256 productId, uint256 reviewIndex, address user) external {
    ...
    function getHighestRatedProduct() external view returns (uint256) {
        ...
        for (uint256 i = 0; i < reviewsCounter; i++) { 
            ...
            if (products[productId].numReviews > 0) {
                uint256 avgRating 
                    = products[productId].totalRating / products[productId].numReviews;
